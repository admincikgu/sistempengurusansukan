...
module.exports = async function handler(req, res) {
  ...
  if (!adminAuthenticated(req)) {
    return send(res, 401, { ok: false, message: "Admin belum disahkan." });
  }
  ...
  try {
    ...
    for (const result of results) {
      const id = String(result.registrationId || "");
      points[id] = (points[id] || 0) + Number(result.points || 0);
    }
    ...
    return send(res, 500, {
      ok: false,
      message: "Admin gagal mendapatkan data daripada MongoDB.",
      error: error.message
    });
  }
};
